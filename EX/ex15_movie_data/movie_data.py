...
class MovieData:
    ...
    def load_data(self, movies_filename: str, ratings_filename: str, tags_filename: str) -> None:
        ...
        :return: None
        """
        filenames = [movies_filename, ratings_filename, tags_filename]
        if len(filenames) != 3:
            raise ValueError
    ...
    def create_aggregate_movie_dataframe(self, nan_placeholder: str = '') -> None:
        ...
        :return: None
        """
        formatted_ratings = self.ratings.drop(labels=['timestamp', 'userId'], axis=1)
        formatted_tags = self.tags.drop(labels=['timestamp', 'userId'], axis=1)
        new_tags = formatted_tags.groupby('movieId').agg({'tag': lambda x: ' '.join(x)})
